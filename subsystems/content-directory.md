...
0. **firstName:** Text up to 20 bytes
1. **isMarried:** Bool
2. **isMale:** Bool
3. **bestFriendId:** Optional reference to another `Person` instance
...
0. `[firstName, isMarried]`
1. `[firstName, isMarried, isMale]`
2. `[firstName, bestFriendId]`
...
0. **firstName (due to 1):** Robert
1. **isMarried (due to 1):** false
2. **isMale (due to 1):** true

Then support for schema 2 is introduced for `Bob`, in which case he could now have property values

0. **firstName (due to 1 and 2):** Robert
1. **isMarried (due to 1):** false
2. **isMale (due to 1):** true
3. **bestFriendId (due to 2):** 3
